...
uintptr_t Aimbot::get_closest_target(float fov)
{
	...
	for (int i = 0; i < 64; i++)
	{
		if (entity.is_valid(entity.get_entbase(i)) && entity.get_team(entity.get_entbase(i)) != offsets.local_team)
		{
...
void Aimbot::aim_at_target(uintptr_t player, int smooth, int bone)
{
	if (player != NULL)
	{
		...
		D3DXVECTOR3 smooth_angles = ClampAngles(entity.get_local_view() + (delta / smooth));
		if (entity.is_valid(player))
			entity.set_local_view(smooth_angles);
	}
}
...
void Aimbot::aimbot()
{
	...
	{
		aim_at_target(get_closest_target(50), 0, 8);
	}
}